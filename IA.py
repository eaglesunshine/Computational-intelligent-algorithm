...
# 定义适应度函数
def fitnessFunction():
    return lambda x: 21.5 + x[0] * np.sin(4 * np.pi * x[0]) + x[1] * \
                    np.sin(20 * np.pi * x[1])
    pass
...
def main(max_iter=500):
    ...
    for iteration in range(max_iter):
        ...
        # 种群解码
        decoded = decodedChromosome(lengthEncode, chromosomesEncoded,
                                    decisionVariables)
        # 得到个体适应度值和个体的累积概率
        ...
        final_decoded = decodedChromosome(lengthEncode, mutationpopulation,
                                          decisionVariables)
        # 适应度评价
        fitnessvalues, cum_individual_proba =getFitnessValue(fitnessFunction(),
                                                              final_decoded)
